...
import { buildUuids, IdentifiedNode, identifyNodes, ReactNode, useInspectElement } from './common'
import { DevTools, DevToolsProps, tabs } from './devtools'
import { OverboardStore, OverboardStoreProps, StoreRef } from './overboard-store'
  ...
  return (
    ...
    >
      ...
      <button
        onClick={() =>
          window.scrollTo({
            top: (document.getElementById('overboard-story')?.offsetTop ?? 0) + window.innerHeight,
            behavior: 'smooth'
          })
        }
        style={{
...
export default function ReplayApplication() {
  ...
  /* Store */
  const [storeState, setStoreState] = useState<OverboardStoreProps['state']>('idle')
  ...
  useIsomorphicLayoutEffect(() => {
    ...
    */
    const nodeLine = () => gsap.utils.selector(devtoolsPanelRef.current)('#node-line')
    ...
    const [storePurchaseLoader] = storeSelector(`#overboard-store-purchase-${storeId} .loader`)
    ...
    floorAndRotateTimeline.current = gsap
      ...
      .to(storeVariables, {
        hoverboardWaveProgress:
          storeVariables.hoverboardAnimationMaxArg * floorAndRotateTimelineDuration,
        floorDisplacement: 1,
        duration: floorAndRotateTimelineDuration,
        ease: 'linear',
        onUpdate: () => {
          const nextProgress =
            storeVariables.hoverboardWaveProgress % storeVariables.hoverboardAnimationMaxArg
    ...
    const flipTimeline3 = Flip.fit(targetStoreRef.current, smallRightStoreAreaRef.current, {
      simple: false,
      duration: 4
    })

    const flipTimeline4 = Flip.fit(devtoolsPanelRef.current, devtoolsAreaRef.current, {
      simple: false,
      duration: 4
    })
    ...
    timeline
      ...
      /* Devtools */
      .addLabel('devtools')
      .set(viewToggleRef.current, { clipPath: 'inset(4px 4px 4px 50% round 4px)' }, 0)
      .set(
        devtoolsTools,
        ...
        >
          ...
          */}
          <div className={s['section-inner']}>
            <AspectBox ratio={1920 / 1080} className={s['store-container']} ref={targetStoreRef}>
              <OverboardStore
                storeId={storeId}
                state={storeState}
                overboardColor="red"
              ...
              >
                ...
                >
                  <div className={clsx('toolbar', s['toolbar'])}>
                    ...
                    >
                      <g clipPath="url(#clip0_806_120)" className="info" fill="#BCBCBC">
                      ...
                      <g clipPath="url(#clip1_806_120)" className="comments" fill="#BCBCBC">
                      ...
                      <g clipPath="url(#clip2_806_120)" className="code" fill="#BCBCBC">
                      ...
                      <g clipPath="url(#clip3_806_120)" className="debugger" fill="#BCBCBC">
                      ...
                      <g clipPath="url(#clip4_806_120)" className="search" fill="#BCBCBC">
